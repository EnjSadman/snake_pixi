...
export class Snake {
  ...
  createSnake() {
    for (let i = 0; i < this.snakeHeight; i++) {
        ...
        snakeBlock.x = (this.app.canvas.width / 2) - this.blockSize * Math.floor(this.snakeWidth / 2);
        snakeBlock.y = (this.app.canvas.height / 2) - this.blockSize * (this.snakeHeight / 2) + i * this.blockSize;
  ...
  checkCollision(newX, newY) {
    if (newX < 0 || newX >= this.app.view.width || newY < 0 || newY >= this.app.view.height) {
      return true;
    }
